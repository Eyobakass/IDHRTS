...
class TaxPaymentViewSet(viewsets.ModelViewSet):
    ...
    def initialize_chapa(self, request):
        ...
        try:
            ...
            if not chapa_key or chapa_key == 'your-chapa-key-here':
                return Response({
                    "error": "Chapa payment gateway is not configured on this server. "
                             "Please contact the administrator or pay at your nearest Woreda office."
                }, status=503)
            ...
            response = requests.post("https://api.chapa.co/v1/transaction/initialize", json=payload, headers=headers, timeout=15)
            ...
            logger.error("Chapa initialization error: %s", e)
